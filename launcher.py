...
from kijiji_scraper import VERSION
...
"""
    ...
    filepath = find_file(['HOME', 'XDG_CONFIG_HOME', 'APPDATA'], [
                         ".kijiji_scraper/config.yaml"], default_content=default_config, create=True)
    # Open with editor
    if os.name == 'nt':
        os.system(filepath)
...
def parse_args():
    ...
    parser.add_argument(
        '--init', '--setup', help="Create config file if doesn't exist and open with default text editor", action='store_true')
    ...
    parser.add_argument('--url', '-u', metavar="URL",
                        help="Kijiji seacrh URLs to scrape", nargs='+', default=None)
    parser.add_argument('--email', '-e', metavar="Email",
                        help="Email recepients", nargs='+',  default=None)
    ...
    parser.add_argument(
        '--all', '-a', help="Consider all ads as new, do not load ads.json file", action='store_true')
    ...
    parser.add_argument(
        '--version', '-V', help="Print Kijiji-Scraper version", action='store_true')
...
def get_ads_summary(ads):
    ...
    for ad_id in ads:
        title_w = len(ads[ad_id]['Title']) + \
            4 if ads[ad_id] and len(ads[ad_id]['Title']) > title_w else title_w
    frow = "{:<%d} {}" % title_w
...
def main():
    ...
    if args.conf:
        ...
        filepath = find_file(
            ['HOME', 'XDG_CONFIG_HOME', 'APPDATA'], [filename])
        if not filepath:
    ...
    if not args.all:
        if args.ads:
            ...
            if not ads_filepath:
                ads_filepath = find_file(['HOME', 'XDG_CONFIG_HOME', 'APPDATA'], [
                                         '.kijiji_scraper/ads.json'], default_content='{}', create=True)
